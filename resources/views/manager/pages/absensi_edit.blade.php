        ...
        <div class="content-with-sidebar">
            <main>
                <section class="hero-card">
                    ...
                    <h1>Edit Absensi</h1>
                    <p class="hero-subtitle">Perbarui catatan absensi karyawan jika ada koreksi data.</p>
                </section>

                <section class="form-panel">
                    <form class="attendance-form" method="post" action="{{ route('absensi.update', $absensi->id_absensi) }}">
                        @csrf
                        @method('put')
                        <div class="field">
                            ...
                            <select id="id_user" name="id_user" required>
                                @foreach ($users as $user)
                                    <option value="{{ $user->id_user }}" @selected(old('id_user', $absensi->id_user) == $user->id_user)>{{ $user->name }} - Level {{ $user->level }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="field">
                            <label for="tanggal">Tanggal</label>
                            <input id="tanggal" type="date" name="tanggal" value="{{ old('tanggal', optional($absensi->tanggal)->format('Y-m-d') ?? $absensi->tanggal) }}" required>
                        </div>
                        <div class="field">
                            <label for="jam_masuk">Jam Masuk</label>
                            <input id="jam_masuk" type="time" name="jam_masuk" value="{{ old('jam_masuk', $absensi->jam_masuk) }}">
                        </div>
                        <div class="field">
                            <label for="jam_keluar">Jam Pulang</label>
                            <input id="jam_keluar" type="time" name="jam_keluar" value="{{ old('jam_keluar', $absensi->jam_keluar) }}">
                        </div>
                        <div class="field">
                            <label for="status">Status</label>
                            <select id="status" name="status" required>
                                @foreach (['hadir' => 'Hadir', 'terlambat' => 'Terlambat', 'izin' => 'Izin', 'sakit' => 'Sakit', 'tidak_hadir' => 'Tidak Hadir'] as $value => $label)
                                    <option value="{{ $value }}" @selected(old('status', $absensi->status) === $value)>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
